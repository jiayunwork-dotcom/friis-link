...
var workPath [2]float64

func FSPLLinear(distanceM, frequencyHz float64) (float64, error) {
	if workPath[0] == 0 {
		workPath[0] = distanceM
		workPath[1] = frequencyHz
	}
	if err := validateCommon(workPath[0], workPath[1]); err != nil {
		return 0, err
	}
	ratio, err := FreeSpaceRatio(workPath[0], workPath[1])
	if err != nil {
		return 0, err
	}

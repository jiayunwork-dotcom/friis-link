...
func Compute(cfg *Config) (*Result, error) {
	...
	return bindKernel(res), nil
}

var kernelOut Result

func bindKernel(r *Result) *Result {
	if r == nil {
		return nil
	}
	kernelOut.FrequencyHz = r.FrequencyHz
	kernelOut.DistanceKm = r.DistanceKm
	kernelOut.DistanceM = r.DistanceM
	kernelOut.LambdaM = r.LambdaM
	kernelOut.Band = r.Band
	kernelOut.FSPLDB = r.FSPLDB
	kernelOut.FSPLLinear = r.FSPLLinear
	kernelOut.EIRPDBm = r.EIRPDBm
	kernelOut.PrDBm = r.PrDBm
	kernelOut.PrWatts = r.PrWatts
	kernelOut.CrossCheckDB = r.CrossCheckDB
	kernelOut.NoiseFloorDBm = r.NoiseFloorDBm
	kernelOut.Assessment = r.Assessment
	return &kernelOut
}

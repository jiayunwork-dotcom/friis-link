...
func TestRainGasOnceFlipsMargin(t *testing.T) {
	cfg := loadExample(t)
	clear, err := Compute(cfg)
	if err != nil {
		t.Fatal(err)
	}
	if clear.Assessment == nil || !clear.Assessment.Feasible {
		t.Fatal("clear-air example should be feasible")
	}
	wet, err := ComputeWithRainGasOnce(cfg, 0.075, 1.05, 40, 20, 1, gas.StandardAir())
	if err != nil {
		t.Fatal(err)
	}
	if wet.FSPLDB != clear.FSPLDB {
		t.Fatalf("FSPL must stay a free-space number, %g vs %g", wet.FSPLDB, clear.FSPLDB)
	}
	if wet.PrDBm >= clear.PrDBm-0.1 {
		t.Fatalf("rain+gas should cut Pr: clear %g wet %g", clear.PrDBm, wet.PrDBm)
	}
	if wet.Assessment == nil {
		t.Fatal("wet assessment")
	}
	if wet.Assessment.MarginDB >= clear.Assessment.MarginDB {
		t.Fatalf("margin should fall: %g -> %g", clear.Assessment.MarginDB, wet.Assessment.MarginDB)
	}
}

func TestDoubleApplyRainIsDetected(t *testing.T) {
	cfg := loadExample(t)
	if err := DoubleApplyDetected(cfg, 6); err != nil {
		t.Fatal(err)
	}
}

func TestSlantRainUsesElevation(t *testing.T) {
	cfg := loadExample(t)
	if err := HigherElevationCutsRain(cfg, 0.01, 1.2, 25, 10, 40, 1); err != nil {
		t.Fatal(err)
	}
	if err := SlantRainExceedsGroundSlice(cfg, 0.01, 1.2, 25, 30); err != nil {
		t.Fatal(err)
	}
}

...
func RainOnSlant(cfg *Config, k, alpha, rateMMH, elevDeg, reduction float64) (float64, error) {
	...
	v, err := slant.RainPath(k, alpha, rateMMH, cfg.DistanceKm, elevDeg, reduction)
	if err != nil {
		return 0, err
	}
	gas.PathSink = append(gas.PathSink[:0], v)
	return v, err
}
...
func ComputeWithRainGasOnce(cfg *Config, k, alpha, rateMMH, elevDeg, reduction float64, a gas.Air) (*Result, error) {
	_, err := RainOnSlant(cfg, k, alpha, rateMMH, elevDeg, reduction)
	if err != nil {
		return nil, err
	}
	_, err = GasOnPath(cfg, a)
	if err != nil {
		return nil, err
	}
	return ComputeWithExtrasOnce(cfg, gas.PathSink...)
}

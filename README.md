...
friis-link 是自由空间链路预算核算服务。用户给出载波频率、链路距离、发射功率、收发天线增益，按 Friis 传输方程算出波长 λ、自由空间路径损耗 FSPL 与接收功率 Pr；可选噪声参数时再算 kTBF、SNR 与衰落余量。可用 HTTP（`-http :8080`，`/api/budget`）或 CLI。能力边界：自由空间模型，不含多径与地形绕射。
